...
function pathSum2Iterative(root, targetSum) {
  if (!root) return [];
  ...
  while (stack.length) {
    let [node, remaining, currentPath] = stack.pop();

    const updatedPath = [...currentPath, node.val];
    remaining -= node.val;
    ...
    if (node.right) {
      stack.push([node.right, remaining, updatedPath]);
    }

    if (node.left) {
      stack.push([node.left, remaining, updatedPath]);
    }
  }
...
console.log("Example 1:", pathSum2Iterative(example1, 22)); // true
console.log("Example 2:", pathSum2Iterative(example2, 6)); // false
console.log("Example 3:", pathSum2Iterative(example3, 5)); // true
